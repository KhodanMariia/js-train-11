...
function setToArray(mySet) {
  ...
  const arrStr = myArr.filter(elem => typeof elem === "string");
...
function removeDuplicatesInPlace(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (set.has(arr[i])) {
      ...
      set.add(arr[i]); // Додаємо унікальний елемент до множини
    }
  }
  //console.log(newArr);
  // Повертаємо множину
...
function iterateSet(set) {
  ...
  for (const entries of iterEntries) {
